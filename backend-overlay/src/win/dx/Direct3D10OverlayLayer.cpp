...
// Description: Windows Direct3D 10 overlay renderer driven
...
bool RenderOverlay(IDXGISwapChain* swapChain, const char* presentPath)
{
    ...
    // DX10 hooks can intercept shared DXGI swap chains - Non-DX10 chains are pass-through
    ...
    if (renderer != WinDxgiOverlayRouter::Renderer::Direct3D10)
    {
        ...
        {
            LYMALINK_LOG("[Direct3D10OverlayLayer][RenderOverlay] pass-through non-DX10 swap chain.");
        }
        return false;
    }
...
{
    // Do not force D3D10 into non-DX10 processes - Watch briefly for late runtime loads
    ...
    {
        d3d10 = GetModuleHandleW(L"d3d10.dll");
        if (d3d10)
        {
            break;
        }
    ...
    LYMALINK_LOG(std::string("[Direct3D10OverlayLayer][Identity] I am DX10; process uses DX10=") + (d3d10 ? "yes; active" : "no; inactive"));
    ...
    if (!d3d10)
    {
        LYMALINK_LOG("[Direct3D10OverlayLayer][InitThread] d3d10.dll not loaded; layer inactive.");
        return 1;
    }

    if (!WinDxgiOverlayCoordinator::Start())
    {
        LYMALINK_LOG("[Direct3D10OverlayLayer][InitThread] DXGI coordinator unavailable.");
        return 1;
    }

    LYMALINK_LOG("[Direct3D10OverlayLayer][InitThread] renderer ready; DXGI coordinator active.");
    return 0;
}
}
...
extern "C" __declspec(dllexport) BOOL WINAPI LymalinkDirect3D10RenderSwapChain(IUnknown* swapChainObject, const char* presentPath)
{
...
extern "C" __declspec(dllexport) BOOL WINAPI LymalinkDirect3D10BeforeResize(IUnknown* swapChainObject)
{
...
extern "C" __declspec(dllexport) HRESULT WINAPI LymalinkDirect3D10AfterResize(IUnknown* swapChainObject, const char* apiName, HRESULT result, IUnknown* const* presentQueue)
{
    (void)apiName;
    (void)presentQueue;
    if (s_shuttingDown.load() || !swapChainObject)
...
extern "C" void LymalinkOverlayOnProcessAttach(HINSTANCE instance)
{
    ...
    LYMALINK_LOG("[Direct3D10OverlayLayer][Identity] I am DX10; process attach.");
